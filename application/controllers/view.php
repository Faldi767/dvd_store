...
class View extends CI_Controller {
	...
	{
		$data['nav'] = "history";
		$data2['history'] = $this->m_user->loadhistory($this->session->userdata("id"))->result();
		$this->load->view('head');
		$this->load->view('navbar', $data);
		$this->load->view('history', $data2);
		$this->load->view('footer');
	}
	...
	function deletecart() {
		...
		if($this->session->has_userdata('nama')) {
		$this->m_user->deletecart($idproduct);
		redirect('view/cart');
		} else {
			redirect('view/register');
			}
	...
	function processcheckout() {
		...
		if($this->session->has_userdata('nama')) {
			$data = array(
				...
				$this->m_user->clearcart($this->session->userdata("id"));
				redirect('view/index');
		} else {
			redirect('view/register');
		}
